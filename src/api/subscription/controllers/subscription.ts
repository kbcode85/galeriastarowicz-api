...
export default factories.createCoreController('api::subscription.subscription', ({ strapi }) => ({
	async createCheckout(ctx: Context) {
		try {
			...
			const price = plan.prices.find(p => p.currency === currency && p.duration === duration && p.isActive)
	...
	async getStatus(ctx: Context) {
		try {
			...
			const activeSubscription = await strapi.db.query('api::subscription.subscription').findOne({
				where: {
					user: user.id,
			...
			const pendingSubscription = await strapi.db.query('api::subscription.subscription').findOne({
				where: {
					user: user.id,
					subscriptionStatus: 'pending_payment',
				},
	...
	async checkExpired(ctx) {
		try {
			const expiredCount = await strapi.service('api::subscription.subscription').checkExpiredSubscriptions()
